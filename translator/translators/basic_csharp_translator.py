...
class PProgramToCSharpTranslator(TranslatorBase):
    ...
    def scoped_do_copy(self, tf):
        class _do_copy(object):
            def __enter__(_self):
                _self.do_copy_old = self.do_copy
                self.do_copy = tf
            def __exit__(_self, exception_type, exception_value, traceback):
                self.do_copy = _self.do_copy_old
        return _do_copy()
    ...
    def visitLocal_var_list(self, ctx):
        if ctx.getChildCount() == 1:
            ...
            ret = ctx.getChild(0).accept(self)
    ...
    def exp_emit_do_copy(self, ctx, out):
        if self.do_copy and ctx.exp_type.clonable:
            return out + ".DeepCopy()"
        else:
            return out
    ...
    def visitExp_getidx(self, ctx):
        with self.scoped_do_copy(False):
            c0 = ctx.getChild(0).accept(self)
        idx = int(ctx.getChild(2).getText())
        return self.exp_emit_do_copy(ctx, "{0}.Item{1}".format(c0, idx + 1))
    ...
    def visitExp_id(self, ctx):
        ...
        if type(ctx.exp_type) is PTypeEvent:
            ...
            return self.exp_emit_do_copy(ctx, identifier)
    ...
    def visitExp_getattr(self, ctx):
        ...
        return self.exp_emit_do_copy(ctx, "{0}.Item{1}".format(c0, idx))
    ...
    def visitExp_keys(self, ctx):
        with self.scoped_do_copy(False):
            c2 = ctx.getChild(2).accept(self)
        return self.exp_emit_do_copy(ctx, "{0}.Keys()".format(c2))
    ...
    def visitExp_values(self, ctx):
        with self.scoped_do_copy(False):
            c2 = ctx.getChild(2).accept(self)
        return self.exp_emit_do_copy(ctx, "{0}.Values()".format(c2))
    ...
    def visitExp_getitem(self, ctx):
        with self.scoped_do_copy(False):
            c0 = ctx.getChild(0).accept(self)
            c2 = ctx.getChild(2).accept(self)
        return self.exp_emit_do_copy(ctx, "{0}[{1}]".format(c0, c2))
