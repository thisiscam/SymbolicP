...
//			KW_PMACHINE_STATES,
                ...
                case 0:
                    {
                        ...
                        for (int i = ret.Statements.Count - 2; i >= 0; i--)
                        {
                            ...
                            {
                                if(stmt.DescendantNodes().Any((arg) => arg.IsKind(SyntaxKind.ReturnStatement))) 
                                {
                                    var block = SyntaxFactory.Block(ret.Statements.Skip(i + 1));
                                    var merge_check = SyntaxFactory.IfStatement(
                                            condition: SyntaxFactory.ParseExpression("PathConstraint.MergedPcFeasible()"),
                                            statement: block
                                    );
                                    ret = ret.WithStatements(RemoveRange(ret.Statements, i + 1, ret.Statements.Count).Add(merge_check));   
                                }
                            }
                        }
                        return ret;
    ...
    private int ret_var_cnt = 0;
        private bool handle_return = false;
            ...
            case 2:
                {
                    ...
                    var ret_var_name = String.Format("vs_ret_{0}", ret_var_cnt);
                var return_cnt = node.DescendantNodes().Count((arg) => arg.IsKind(SyntaxKind.ReturnStatement)) ;
                     handle_return = !(return_cnt == 0 || (return_cnt == 1 && (node.Body.IsKind(SyntaxKind.ReturnStatement) || (node.Body.IsKind(SyntaxKind.Block) && (node.Body as BlockSyntax).Statements.Last().IsKind(SyntaxKind.ReturnStatement)))));
            ...
            case 3:
                {
                    ...
                    if (node.Else != null)
                    {
                        ...
                        ret = ret.AddStatements(SyntaxFactory.ParseStatement(String.Format("{0}.MergeBranch();", cond_name)));
                    return ret;
                }
            default:
                {
    ...
    public override SyntaxNode VisitForStatement(ForStatementSyntax node)
    {
        ...
        switch (pass)
        {
            case 3:
                {
                    var cond = SyntaxFactory.InvocationExpression(SyntaxFactory.MemberAccessExpression(SyntaxKind.SimpleMemberAccessExpression, node.Condition.Accept(this) as ExpressionSyntax, SyntaxFactory.IdentifierName("Loop")));
                    var loop = (base.VisitForStatement(node) as ForStatementSyntax).WithCondition(cond);
                    var ret = SyntaxFactory.Block(
                            SyntaxFactory.ParseStatement("PathConstraint.BeginLoop();"),
                            loop
                        );
                    return ret;
                }
            ...
            case 2:
                {
                        if (handle_return)
                        {
                            ...
                            {
                                var ret_var_name = String.Format("vs_ret_{0}", ret_var_cnt);
                                return SyntaxFactory.ParseStatement(String.Format("{0}.RecordReturn({1});", ret_var_name, node.Expression.Accept(this).ToFullString()));
                            }
